 ...
 */
class KMeansModel(centroids: List[LabeledPoint]) {
  def label(point: LabeledPoint) = KMeansModel.findNearestCentroid(centroids, point)
}
...
object KMeansModel {
  ...
  def error(labelToCentroidMap: Map[Double, LabeledPoint], points: List[LabeledPoint]): Double = {
    points.map(p => sqdist(p.features, labelToCentroidMap(p.label).features)).sum
  }
}
...
object tests extends App {
  override def main (args: Array[String]) {
    ...
    val points = ClassicKMeansModeler.generateInitialCentroids(5, List((0.0, 10.0), (0.0, 10.0), (0.0, 10.0)))
    points.map(println)
